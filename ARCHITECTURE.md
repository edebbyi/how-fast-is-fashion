...
- **Not yet root-caused.** The working hypothesis going into this curation round (from v8/v14: `quiet_luxury`'s over-representation was a curation-quality artifact that more/better references would fix) does not appear to hold on its own - the imbalance grew even as curation broadened. Possible explanations, none confirmed: (a) `quiet_luxury`'s new references may still skew toward generic neutral-clothing photos that match a wide swath of the catalog almost by default, widening its net rather than narrowing it; (b) the office_siren/mob_wife additions may be correct but still too few relative to quiet_luxury's now-larger reference count to shift k-NN voting outcomes at catalog scale; (c) some interaction with the mostly-flat precision-vs-threshold curve (v12) that hasn't been re-examined at the new corpus size. Needs actual investigation, not more curation-by-feel, before the next round.
- **Recommendation for next session:** before approving more `quiet_luxury` candidates from the pending queue, run `scripts/eval_trend_classifier.py`'s confusion-matrix output (or an ad hoc check) specifically on what's driving `quiet_luxury`'s k-NN wins at catalog scale - is it winning on genuinely discriminating signal, or still on generic/neutral-clothing similarity the way v8 first found for the original 43-image set.
